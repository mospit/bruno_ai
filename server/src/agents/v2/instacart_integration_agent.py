...
class InstacartIntegrationAgentV2(BaseAgent):
    ...
    async def create_optimized_shopping_list(self, items: List[Dict], budget: float, preferences: Dict = None) -> Dict[str, Any]:
        ...
        # Generate Bruno's response about the shopping experience
        bruno_message = await self._generate_bruno_shopping_message(
            shopping_lists, instacart_experience, budget
        )
        
        return {
            "success": True,
            "bruno_message": bruno_message,
...
class RateLimiter:
    ...
    async def _generate_bruno_shopping_message(self, shopping_lists: Dict, 
                                             instacart_experience: Dict, budget: float) -> str:
        """Generate Bruno's personalized shopping message"""
        
        total_cost = instacart_experience.get("total_cost", 0)
        savings = instacart_experience.get("savings", 0)
        stores_used = len(shopping_lists)
        
        # Calculate budget performance
        if total_cost <= budget:
            savings_amount = budget - total_cost
            if savings_amount > budget * 0.15:  # Saved more than 15%
                message = f"Bada-bing! Look at that - I got ya everything for ${total_cost:.2f} and saved ya ${savings_amount:.2f}! That's what I'm talkin' about - ya gonna eat great and still have money left over!"
            elif savings_amount > 0:
                message = f"Hey, nice work! Came in at ${total_cost:.2f}, which is ${savings_amount:.2f} under budget. Bruno's got ya covered with some solid deals!"
            else:
                message = f"Perfect! Right on target at ${total_cost:.2f}. Ya budget's locked in tight, and we found some good deals along the way."
        else:
            overage = total_cost - budget
            message = f"Listen, we're a little over at ${total_cost:.2f} - that's ${overage:.2f} more than planned. But lemme tell ya, these prices are still solid for what ya gettin'. Trust me on this one."
        
        # Add store optimization details
        if stores_used > 1:
            message += f" I optimized across {stores_used} stores to get ya the best deals."
        
        # Add savings highlight
        if savings > 0:
            message += f" Plus, I found ya ${savings:.2f} in extra savings with current deals and coupons!"
        
        # Add delivery info
        delivery_options = instacart_experience.get("delivery_options", {})
        if delivery_options.get("fastest_delivery"):
            message += f" And ya can get this delivered as fast as {delivery_options['fastest_delivery']} if ya need it quick."
