...
class TokenManagementTestSuite:
    ...
    async def test_compression_functionality(self):
        ...
        with patch('redis.asyncio.from_url') as mock_redis, \
            ...
            # Ensure the mock response is properly set up for async
            mock_response = MagicMock()
            mock_content = MagicMock()
            mock_content.text = compressed_response
            mock_response.content = [mock_content]
            
            # Use AsyncMock for async API calls
            mock_anthropic_client.messages.create = AsyncMock(return_value=mock_response)
            
            # Replace the actual client with our mock
            token_manager.anthropic_client = mock_anthropic_client
